    ...
    with tab1:
        ...
        if view_option == "All Appointments":
            if st.button("Show All Appointments"):
                success, msg, appts = appt_manager.list(manager, "nurse", username, scope="all", upcoming_only=False, date=None, status=None, patient_id=None, doctor_id=None, appt_id=None)
                if success:
                    st.success(msg)
                    if appts:
                        for appt in appts:
            ...
            if st.button("Search"):
                if appt_id:
                    success, msg, appt = appt_manager.list(manager, "nurse", username, scope="all", upcoming_only=False, date=None, status=None, patient_id=None, doctor_id=None, appt_id=appt_id)
                    if success:
                        st.success(msg)
                        st.json(appt)
                    else:
            ...
            if st.button("Search"):
                if patient_id:
                    success, msg, appts = appt_manager.list(manager, "nurse", username, scope="all", upcoming_only=False, date=None, status=None, patient_id=patient_id, doctor_id=None, appt_id=None)
                    if success:
                        st.success(msg)
                        if appts:
                            for appt in appts:
            ...
            if st.button("Search"):
                ...
                success, msg, results = appt_manager.list(manager, "nurse", username, scope="all", upcoming_only=False, date=date_str, status=None, patient_id=None, doctor_id=None, appt_id=None)
                if success:
                    st.success(msg)
                    if success:
                        if results:
