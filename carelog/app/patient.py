...
class PatientUser(User):
    def __init__(self, p_id, username, password, name, gender, address, email, contact_num, date_joined, p_record):
        self.p_id = p_id
        super().__init__(username, password, name, gender, address, email, contact_num, date_joined)
        self.p_record = p_record or []

class PatientRecord():
    def __init__(self, pr_record_id, p_id, pr_timestamp, pr_conditions, pr_prediction_result, pr_confidence_score):
